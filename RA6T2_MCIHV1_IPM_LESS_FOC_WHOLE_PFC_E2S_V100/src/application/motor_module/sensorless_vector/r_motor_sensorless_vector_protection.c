...
*                 f4_overspeed_limit_rad - The speed[rad/s] threshold of the over-speed error, should be a positive value
* Return Value  : The flag of over-speed error
***********************************************************************************************************************/
                                                                        ...
                                                                        float f4_overcurrent_limit)
{
    ...
    if (fabsf(f4_iu) > f4_overcurrent_limit)
    {
        u2_temp0 |= 0x0001;
    }
    else
    ...
    if (fabsf(f4_iv) > f4_overcurrent_limit)
    {
        u2_temp0 |= 0x0001;
    }
    else
    ...
    if (fabsf(f4_iw) > f4_overcurrent_limit)
    {
        u2_temp0 |= 0x0001;
    }
    else
...
/***********************************************************************************************************************
* Function Name : motor_sensorless_vector_stall_detection_error_check
* Description   : Checks stall detection
* Arguments     : u1_stall_detected - The flag of stall detection
* Return Value  : The flag of stall detection error
***********************************************************************************************************************/
static inline uint16_t motor_sensorless_vector_stall_detection_error_check(uint8_t u1_stall_detected)
{
    uint16_t u2_temp0;

    u2_temp0 = MOTOR_SENSORLESS_VECTOR_ERROR_NONE;

    if (MTR_FLG_SET == u1_stall_detected)
    {
        u2_temp0 = MOTOR_SENSORLESS_VECTOR_ERROR_STALL_DETECTED;          /* Stall detection error */
    }
