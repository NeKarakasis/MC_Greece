...
typedef struct st_motor_speed_instance_ctrl
{
    ...
    /* variables for torque mode with speed limit */
    motor_speed_control_mode_t e_control_mode;
    uint8_t u1_torque_state;
    float f_torque_ref_mnm;
    float f_final_iq_torque;                                          ///< the final command of iq
...
fsp_err_t RM_MOTOR_SPEED_TorqueReferenceSet(motor_speed_ctrl_t * const p_ctrl, float const torque_reference_mnm);

fsp_err_t RM_MOTOR_SPEED_ControlModeSet(motor_speed_ctrl_t * const p_ctrl, motor_speed_control_mode_t const mode);
